    ...
    <>
      <header 
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          isScrolled ? 'bg-[#131921]/95 backdrop-blur-xl shadow-lg' : 'bg-[#131921]'
        }`}
      ...
      >
        <div className="h-full px-4 flex items-center gap-4 max-w-[1920px] mx-auto">
          <Link 
            ...
            style={{ transform: isScrolled ? 'scale(0.9)' : 'scale(1)', transition: 'transform 0.3s cubic-bezier(0.16, 1, 0.3, 1)' }}
          >
          ...
          {user?.role !== 'seller' ? (
            <div ref={searchRef} className={`flex-1 max-w-2xl mx-4 relative transition-all duration-500 ${isSearchFocused ? 'scale-[1.02]' : 'scale-100'}`}>
              <form onSubmit={handleSearch}>
                <div className="flex rounded-lg overflow-hidden shadow-lg">
                  ...
                  />
                  <Button type="submit" className="rounded-none bg-[#febd69] hover:bg-[#f90] text-[#131921] px-4 h-10 transition-all duration-200">
          ...
          <div className="flex items-center gap-1 sm:gap-3">
            {isAuthenticated ? (
              ...
              <Link to="/login" className="text-white/90 hover:text-white transition-colors px-2 py-1 rounded hover:bg-white/10">
            ...
            {user?.role !== 'seller' && (
              <Link to={isAuthenticated ? "/orders" : "/login"} className="hidden md:block text-white/90 hover:text-white transition-colors px-2 py-1 rounded hover:bg-white/10">
                <p className="text-xs text-white/70">Returns</p>
                <p className="text-sm font-medium">& Orders</p>
              </Link>
            )}

            {user?.role !== 'seller' && isAuthenticated && (
              <Link to="/wishlist" className="hidden md:flex items-center text-white/90 hover:text-white transition-colors px-2 py-1 rounded hover:bg-white/10">
      ...
      {user?.role !== 'seller' && (
        <nav className="fixed top-[60px] left-0 right-0 z-40 bg-[#232f3e] text-white hidden lg:block transition-all duration-300"
          style={{ top: isScrolled ? '50px' : '60px', opacity: isScrolled ? 0.95 : 1 }}
        >
          <div className="px-4 py-2 flex items-center gap-6 max-w-[1920px] mx-auto overflow-x-auto">
            <button onClick={() => setIsMobileMenuOpen(true)} className="flex items-center gap-1 font-medium hover:text-[#febd69] transition-colors whitespace-nowrap">
              <Menu className="w-4 h-4" />All
            </button>
            {categories.map(cat => (
              <Link key={cat.id} to="/products" onClick={() => setFilters({ category: cat.id as any })} className="text-sm hover:text-[#febd69] transition-colors whitespace-nowrap">
                {cat.name}
              </Link>
            ))}
            <Link to="/products" className="text-sm hover:text-[#febd69] transition-colors whitespace-nowrap">Today's Deals</Link>
            <Link to="/products" className="text-sm hover:text-[#febd69] transition-colors whitespace-nowrap">New Arrivals</Link>
          </div>
        </nav>
      )}
          ...
          <div className="absolute left-0 top-0 bottom-0 w-80 bg-white shadow-2xl overflow-y-auto animate-slide-in-left">
            <div className="bg-[#232f3e] text-white p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <span className="font-medium text-lg">{isAuthenticated ? `Hello, ${user?.name}` : 'Hello, Sign in'}</span>
              </div>
